...
#														if ($deploymentJob.job.status -like "inProgress") {
															...
															foreach ($task in $deploymentJob.tasks) {
																...
																$tmpobjectRelease | Add-Member NoteProperty IssueID                $issueCount
																...
																if ($task.startTime) {
																	...
																	$tmpobjectRelease | Add-Member NoteProperty Start_Time         ([DateTime]::Parse($deployStep.queuedOn)).toString("MM/dd/yyyy HH:mm:ss")
																	$tmpobjectRelease | Add-Member NoteProperty Start_TimeZ        ([DateTime]::Parse($deployStep.queuedOn)).ToUniversalTime().toString("MM/dd/yyyy HH:mm:ss")
																}
																if ($task.finishTime) {
																	if ($task.startTime) {
																	...
																	$tmpobjectRelease | Add-Member NoteProperty Finish_Time        (get-date).toString("MM/dd/yyyy HH:mm:ss")
																...
																if ($deployStep.queuedOn) {
																	if ($task.startTime) {
																		...
																		$tmpobjectRelease | Add-Member NoteProperty Wait_Time      0
																	}
																...
																foreach ($issue in $task.issues) {
																	...
																	if ($issue | where {$_.issueType -like "Error"}) {
																		...
																		$tmpObjectTask | Add-Member NoteProperty IssueID                $issueCount
																		...
																		if ($task.startTime) {
																			...
																			$tmpObjectTask | Add-Member NoteProperty Start_Time         ([DateTime]::Parse($deployStep.queuedOn)).toString("MM/dd/yyyy HH:mm:ss")
																			$tmpObjectTask | Add-Member NoteProperty Start_TimeZ        ([DateTime]::Parse($deployStep.queuedOn)).ToUniversalTime().toString("MM/dd/yyyy HH:mm:ss")
																		}
																		if ($task.finishTime) {
																			if ($task.startTime) {
																				$tmpObjectTask | Add-Member NoteProperty Elapsed_Time   (new-TimeSpan -start $task.startTime -end $task.finishTime).TotalMinutes
																			} else {
																				$tmpObjectTask | Add-Member NoteProperty Elapsed_Time   0
																			}
																			...
																			$tmpObjectTask | Add-Member NoteProperty Finish_Time        (get-date).toString("MM/dd/yyyy HH:mm:ss")
																			$tmpObjectTask | Add-Member NoteProperty Finish_TimeZ       (get-date).ToUniversalTime().toString("MM/dd/yyyy HH:mm:ss")
																			$tmpObjectTask | Add-Member NoteProperty Elapsed_Time       0
																		}
																		...
																		if ($deployStep.queuedOn) {
																			if ($task.startTime) {
																				...
																				$tmpObjectTask | Add-Member NoteProperty Wait_Time      0
																			}
...
"@
			$ids = (invoke-RestMethod -Uri http://$($strElasticSearchServer):9200/releaseinfo/_search?size=1000 -Method POST -Body $esString -ContentType "application/json").hits.hits._id
			if ($ids) {
				$esString = ""
				foreach ($id in $ids) {
...
{"ID": "releaseinfo","LastRunTime": "$($thisRunDateTime)"}
