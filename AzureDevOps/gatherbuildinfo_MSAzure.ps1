...
function main {
	...
	if ($ENV:strDefinitions) {
		...
		$strDefinitions = "Evoke", "Camera", "Mira", "OMG.Shared.Tools", "Paint", "Story", "Video", "Canvas3d", "3MFSDK", "GLTFSDK-CI", "Microsoft.Photos", "Photos", "GLTF"
	}
	...
	if (!($minutesBack = $ENV:minutesBack)) {
		try {
			$tmp = invoke-RestMethod -Uri http://$($strMetricsEsServer):9200/timestamp/data/buildinfo
				...
				foreach ($build in $builds) {
					...
					if (!($build.status -like "inProgress")) {
						...
						if ($build.queueTime -and $build.startTime -and $build.finishTime -and ($build.definition.name -like "$($definition)*")) {
							...
							$tmpobject | Add-Member NoteProperty BuildDefID       $id
