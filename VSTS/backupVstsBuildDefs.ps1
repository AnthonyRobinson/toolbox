...
function main {
	...
	$backupFolder			= "buildDefBackup"
	
	if (!(test-path $backupFolder)) {
		mkdir $backupFolder
	}

	foreach ($defName in "Evoke.App", "Camera", "Mira", "OMG.Shared") {
	
		$buildIds = (Invoke-RestMethod -Uri "$projectUrl/_apis/build/definitions?name=$($defName)*&api-version=2.1" -Headers $headers).value.id
		
		foreach ($buildId in $buildIds) {
			$build = Invoke-RestMethod -Uri "$projectUrl/_apis/build/definitions/$($buildId)?api-version=2.1" -ContentType "application/json" -Headers $headers
			write-Host "*****" $($build.name) "*****"
			$build | convertto-json -depth 100 | out-file "BuildDefBackup\$($buildId) $($build.name).json" -encoding ascii
		}
	}
}
